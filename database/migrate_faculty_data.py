...
def get_db_config():
    """Get database configuration from user input or environment"""
    import getpass
    ...
    database = os.getenv('DB_NAME', 'nitgoa_db')
    
    # If password is not set, ask user
    if not password:
        print("Please enter your MySQL database credentials:")
        if host == 'localhost':
            host = input(f"MySQL host [{host}]: ") or host
        if user == 'root':
            user = input(f"MySQL username [{user}]: ") or user
        password = getpass.getpass("MySQL password: ")
        if database == 'nitgoa_db':
            database = input(f"Database name [{database}]: ") or database
